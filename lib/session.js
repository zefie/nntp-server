...
function Session(server, stream) {
  ...
  this.write((server.options.allow_posting) ? status._200_SRV_READY_RW : status._201_SRV_READY_RO);
...
Session.prototype.parse = function (data) {
	...
	if (input == "POST") {
		if (this.posting_allowed) {
			this.is_posting = true // handle post seperately
			enqueue(this, new Command(() => Promise.resolve(status._340_SEND_ARTICLE), null))
			return;			
		} else {
			enqueue(this, new Command(() => Promise.resolve(status._440_NO_POST_PERM), null))
			return;
		}
	}
	if (!this.is_posting) {
	  ...
	  enqueue(this, new Command(() => Promise.resolve(this.server.commands[cmd].run(this, input)), input));
		...
		if (data.toString().replace(/\r?\n$/, '') == '' && !this.recv_post_body) {
			this.recv_post_body = true;
			return;
		}

		if (input === ".") {
			delete this.is_posting;
			delete this.recv_post_body;
			if (this.server._postArticle(this)) enqueue(this, new Command(() => Promise.resolve(status._240_ARTICLE_RECV_OK), null))
			else enqueue(this, new Command(() => Promise.resolve(status._441_POSTING_FAILED), null))
			delete this.post_data;
			return;
		} 		
		
		if (!this.recv_post_body) {
			// headers
			if (!this.post_data.headers) this.post_data.headers = {}
			var header = data.toString().split(/\: (.*)/s)
			this.post_data.headers[header[0]] = header[1];
		} else {
			if (!this.post_data.body) this.post_data.body = [];
			this.post_data.body.push((data) ? data.toString() : '');
		}
	}
};
